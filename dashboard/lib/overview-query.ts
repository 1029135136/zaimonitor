...
export async function queryOverview(
  ...
  try {
    endpointFamily = normalizeEndpointFamily(params.endpointFamily);
  } catch (exc) {
  ...
  try {
    ...
    const trendDocsV4 = await collection
    ...
    const models = (
      await collection
        .distinct<string>("model", matchModels)
        .catch(() => ["glm-4.7", "glm-5"])
    ).filter((m): m is string => typeof m === "string" && m.length > 0);

    if (models.length === 0) {
      models.push("glm-4.7", "glm-5");
    }
